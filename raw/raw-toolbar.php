<nav id="eze-toolbar">
    <ul>
        <li><a href="?dashboard"><b class="eze-settings"></b>Username</a></li>
        <li><a href="?addcontent"><b class="eze-create"></b>Create</a>
        <li><a href="#"><b class="eze-document"></b><span class="eze-status published"></span>Edit</a>
            <div class="slide-in group">
                    <a href="?view=attributes"><b class="eze-document"></b>Edit page</a>
                    <a href="?publish"><b class="eze-publish"></b><span class="eze-status published"></span>Publish</a>
                    <a href="?view=layout"><b class="eze-layout"></b>Layout</a>
            </div>
        </li>
        <li><a href="#"><b class="eze-search"></b>Find</a>
            <div class="slide-in quick-action search">
                <div class="input-prepend">
                <span class="add-on"><i class="icon-search"></i></span><input id="prependedInput" placeholder="Type to search…" size="16" type="text">
              </div>
               <button class="btn">View all content</button>
            </div>
        </li>
        <li><a href="?activity"><b class="eze-activity"></b>Activity</a></li>
        <li><a href="#eze-mediabase"><b class="eze-media"></b>Media</a></li>
        <li><a href="?view=sitemap"><b class="eze-sitemap"></b>Sitemap</a></li>
        <li><a href="?devices"><b class="eze-devices"></b>Devices</a></li>
        <li><a href="#"><b class="eze-more"></b>More</a></li>
    </ul>
</nav>